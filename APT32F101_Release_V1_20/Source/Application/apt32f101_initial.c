...
//EXI0_INT= EXI0,EXI1_INT= EXI1, EXI2_INT=EXI2~EXI3, EXI3_INT=EXI4~EXI8, EXI4_INT=EXI9~EXI13    
    ...
    GPIOA0_EXI_Init(EXI7);                                   	 //PA0.7 as input
    EXTI_trigger_CMD(ENABLE,EXI_PIN7,_EXIFT);                  //ENABLE falling edge
	EXTI_trigger_CMD(ENABLE,EXI_PIN7,_EXIRT);                  //ENABLE rising edge
    EXTI_interrupt_CMD(ENABLE,EXI_PIN7);                	   //enable EXI
    GPIO_EXTI_interrupt(GPIOA0,0b00000010000000);				  //enable GPIOA07 as EXI
...
void ADC12_CONFIG(void)
{
    ...
    ADC12_Configure_Mode(ADC12_12BIT , ADC12_8CYCLES , Continuous_mode , 2 , 2);        //select 12BIT ADC ,SAMPLEING CYCLE=3 , one shot mode, ADC_CLK=PCLK/2*1 , NumConver=2
...
void GTC1_CONFIG(void)
{
    ...
    //GTC_ControlSet_Configure(GTC1,GTC_ControlSet_CAPT_TCAP1,ENABLE);            //capture up enable
...
void GTC2_CONFIG(void)
{
	...
	//GTC_ControlSet_Configure(GTC2,GTC_ControlSet_CAPT_R,ENABLE);                //capture down enable
    GTC_ControlSet_Configure(GTC2,GTC_ControlSet_CAPT_TCAP2,ENABLE);            //capture up enable
...
void GTC3_CONFIG(void)
{
    ...
    //GTC_ControlSet_Configure(GTC3,GTC_ControlSet_CAPT_TCAP3,ENABLE);            //capture up enable
...
{
    I2C_DeInit();                                           //clear all I2C Register
    I2C_Slave_Init(I2C_G2,FAST_MODE,0x4A,0x20,0xAC);        //slave address=0xac(8bit)，standard mode，speed=sysclock/0x138,0x78=setup time
    I2C_Int_Enable();                                       //use interrupt mode 
}
/*************************************************************/
//CORET Functions
